    ...
    function openChat()
    {
        ...
        var win = window.open(homeUrl + '/Chat.aspx?chatId='+chatId+'&aid='+accountId+'', 'chat', 'toolbar=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=0,width=603,height=510');
        win.focus();
        win.opener = window;
        return ;
    }
    ...
    function hitWebSite() {
        ...
        if (vid != null) {
            $.getJSON(homeUrl + "/VisitorHandler.ashx", { "Action": "1", "VisitorId": vid }, function(json) {
                if (jQuery.trim(json.InviteChatId) != "") {
    ...
    function acceptInvitation() {
        $.get(homeUrl + "/VisitorHandler.ashx", { "Action": "2", "ChatId": chatId }, function(res) {
        //alert(res);
        });
        openChat();
    }
    ...
    function declineInvititation() {
        $.get(homeUrl + "/VisitorHandler.ashx", { "Action": "3", "ChatId": chatId }, function(res) {
        //alert(res);
        }); 
        invite_panelClose();
    }