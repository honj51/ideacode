...
namespace VisualAsterisk.ExceptionManagement
{
    ...
    public class ErrorCaptureUtils
    {
        ...
        public static void SendError(Exception ex, string licenseKey, string licenseName, string version,  string url,string user,string pwd)
        {
            SendError(ex, licenseKey, licenseName, version, url, false,user,pwd);
        }
