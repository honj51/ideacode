...
public partial class GetBackPassword : System.Web.UI.Page
{
    ...
    protected void btnGet_Click(object sender, ImageClickEventArgs e)
    {
        if (Session["createRandom"] != null)
        {
            ...
            {
                Operator oper = OperatorsManager.GetPasswordByAccountNameLoginNameAndEmail(this.txtAccountLoginName.Text.Trim(), this.txtOperatorLoginName.Text.Trim(), this.txtEmail.Text.Trim());
                if (oper != null)
                {
                    ...
                    string title="LiveSpport在线客服系统提示您：";
                    string content = "尊敬的客服 " + oper.NickName + " 您好!\n   您的账号为："+oper.LoginName+",您的密码为：" + oper.Password + ",请您牢记,感谢您一直以来对我们公司产品的支持与厚爱!\n \n \n \n \n \n \n \n                                                        如需帮助请浏览我们公司的官方网站 http://www.zxkefu.cn";
                    bool b =LiveSupport.BLL.Util.SendEmail(toEmail, email, emailPwd, emailSmtp, title, content);
                    if (b)
                    {
