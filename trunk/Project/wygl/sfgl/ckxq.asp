...
<table width="90%" border="0" align="center" cellpadding="0" cellspacing="0">
        ...
        <tr>
          <td height="30">&nbsp;&nbsp;&nbsp;合同编号：<a href="sjlr_xq.asp?id=<%=request.QueryString("id")%>&page=<%=request.QueryString("page")%>"><%=rs1("编码")%></a>&nbsp;&nbsp;工业园名称：<%=rs1("所属工业园")%>&nbsp;&nbsp;房产类型：<%=rs1("房产类型")%>&nbsp;&nbsp;所属房产：<%=rs1("所属房产")%>&nbsp;</td>
        </tr>
        <tr>
...
%>
        ...
        <tr>
		  ...
		  总金额：￥<%=FormatNumber(rs00125("总费用"),2,-1)%>&nbsp;&nbsp;
		  上次结余：￥<%=FormatNumber(rs00125("余额"),2,-1)%>&nbsp;&nbsp;
		  需要交费金额：￥<%=FormatNumber(rs00125("总费用")-rs00125("余额"),2,-1)%>&nbsp;&nbsp;</td>
        </tr>
</table>
	  <table width="90%" border="0" align="center" cellpadding="0" cellspacing="0">
        <tr>
