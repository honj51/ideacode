...
<a onMouseOver="return hidestatus()" href="?Operation=lb">录入列表</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</strong></td>
  </tr>
  <tr>
  ...
  <tr>
	...
	<table width="100%"  style="border:1px solid  #00376F; margin-top:10px" cellspacing="0" cellpadding="0">
      <tr>
        ...
        <td align="center" valign="middle" style="border-bottom:1px solid #00376F; border-right:1px solid #00376F"><strong>录入月份</strong></td>
...
elseif khmc="" and gyy="" and  fclx<>"" and ssfc<>"" then
sql0="select * from zpgl  where 房产类型='"&fclx&"' and  所属房产='"&ssfc&"' 合同结束时间>'"&date()&"' and 合同开始时间<'"&date()&"' order by 所属工业园,房产类型,所属房产 asc"
...
elseif zt=0 then
   	response.Write("<span style='color:#666666'>无需录入</span>")
end if
%>
	</td>
        <td height="30" align="center" valign="middle"style="border-bottom:1px solid #00376F;">
		<a href="sjlr_xq.asp?id=<%=rs0("id")%>&page=<%=epage%>" >查看详情</a>&nbsp;</td>
      </tr>
<%
rs0.movenext()
next
